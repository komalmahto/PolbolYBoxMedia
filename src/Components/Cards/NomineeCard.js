  ...
  return (
    <>
      ...
      <div className='nominee-card'>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-evenly', marginTop: '1rem', alignItems: 'center' }}>{<PlayCircleOutlined onClick={() => { setOpen(true) }} />}{<Button style={{ cursor: 'pointer', backgroundColor: 'rgb(172, 7, 75)', color: 'white' }} onClick={vote}>Vote Now</Button>} {<a style={{ fontSize: '2rem' }} target='_blank' rel={'external'} href={p.weblink}><ArrowRightOutlined /></a>}</div>
      </div>
    </>
  );
};
