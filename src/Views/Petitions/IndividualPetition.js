    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.main}>
          <div>
            ...
            <p className={styles.imgtext}>
              ...
              {petitionData &&
                console.log(
                  stateToHTML(convertFromRaw(JSON.parse(petitionData.content)))
                )}
              <div
                dangerouslySetInnerHTML={{
                  __html:
                    petitionData &&
                    stateToHTML(
                      convertFromRaw(JSON.parse(petitionData.content))
                    ),
                }}
              />
          ...
          <div className={styles.box}>
            <p className={styles.one}>
              {petitionData && petitionData.signaturesReceived
                ? petitionData.signaturesReceived
                : 0}{" "}
              have signed. Let’s get to{" "}
              {petitionData ? petitionData.expectedSignature : ""}!
            </p>
            {/* <ProgressBar
          ...
          /> */}
            <ProgressBar
              bgcolor="#84855d"
              progress={
                ((petitionData ? petitionData.signaturesReceived : 0) * 100) /
                (petitionData ? petitionData.expectedSignature : 0)
              }
              height={15}
            />
        ...
        <AiOutlineUser/><p ><span className={styles.three}>Manveer Dodani</span> signed 55 minutes ago</p> */}
            ...
            {auth.token && petitionData && !petitionData.signedByMe ? (
              <button onClick={signPetitionHandler} className={styles.btn}>
                Sign In This Petition
              </button>
            ) : (
              auth.token && (
                <button className={styles.btn}>Signature Received</button>
              )
            )}
            {!auth.token && (
              <button
                onClick={() => toast("Login to sign petition!!")}
